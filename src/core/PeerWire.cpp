...
namespace burlak::core
{
    ...
    namespace
    {
        ...
        static_assert(std::is_trivially_copyable_v<HelloHeader> && sizeof(HelloHeader) == 48);
    ...
    std::optional<PeerHello> decodePeerHello(std::span<const std::byte> bytes)
    {
        ...
        if (!header || header->version != peerProtocolVersion || header->process == 0 || header->echoNonce == 0 ||
            header->nonce == 0 || !nativeValue(header->tool) || !nativeValue(header->host)) {
            return std::nullopt;
        }
        return PeerHello{.process = header->process,
                         ...
                         .echoNonce = header->echoNonce,
                         .nonce = header->nonce};
    }

    std::vector<std::byte> encodePeerDrop(const Drop &drop)
    {
        if (drop.paths.empty() || drop.paths.size() > peerDropMaximumPaths || drop.nonce == 0) {
            return {};
        }
        std::size_t size = peerDropHeaderSize;
        for (const auto &path : drop.paths) {
            const auto availableCharacters = (peerDropMaximumBytes - size) / sizeof(wchar_t);
            if (path.size() >= availableCharacters) {
                return {};
            }
            size += (path.size() + 1) * sizeof(wchar_t);
        }
    ...
    std::optional<Drop> decodePeerDrop(std::span<const std::byte> bytes)
    {
        ...
        return Drop{.paths = std::move(paths),
                    .at = {header->x, header->y},
                    .effect = static_cast<Effect>(header->effect),
                    .nonce = header->nonce};
    }
